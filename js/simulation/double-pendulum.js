...
function rk5(Z, diff, h) {
  ...
  var k5 = diff(
    addWeightedZ(1, Z, (h * -3) / 16, k2, (h * 6) / 16, k3, (h * 9) / 16, k4)
  );
  var k6 = diff(
    addWeightedZ(
      1,
...
function rk8(Z, diff, h) {
  ...
  var k6 = diff(
    addWeightedZ(1, Z, h * a61, k1, h * a63, k3, h * a64, k4, h * a65, k5)
  );
  var k7 = diff(
    addWeightedZ(
      1,
  ...
  var k8 = diff(
    addWeightedZ(1, Z, h * a81, k1, h * a85, k5, h * a86, k6, h * a87, k7)
  );
  var k9 = diff(
    addWeightedZ(
      1,
...
DoublePendulum.prototype.diff = function(Z) {
  ...
  ang1_accel =
    ...
    b11 *
    (g * (b2 + M2) * Math.sin(b3) +
      g * M2 * Math.sin(b3 - 2 * b4) +
      2 * M2 * b20 * (L1 * b21 * b23 + L2 * b26));
  ...
  ang2_accel =
    ...
    b11 *
    b20 *
    ((M1 + M2) * (g * Math.cos(b3) + L1 * b23) + L2 * M2 * b21 * b26);
...
DoublePendulum.prototype.draw = function() {
  ...
  var KE =
    (1 / 2) * (M1 + M2) * Math.pow(L1, 2) * Math.pow(vang1, 2) +
    (1 / 2) * M2 * Math.pow(L2, 2) * Math.pow(vang2, 2) +
    M2 * L1 * L2 * vang1 * vang2 * Math.cos(ang1 - ang2);
