  ...
  var TP = new TriplePendulum(90, 0, 180, 0, 90, 0, 0, 0, 5, 5, 5, 1, 1, 1);
  ...
  function TriplePendulum(ang1, vang1, ang2, vang2, ang3, vang3, x, y, L1, L2, L3, M1, M2, M3)
  {
    ang1 = ang1/180*Math.PI;
    ang2 = ang2/180*Math.PI;
    ang3 = ang3/180*Math.PI;
    this.Z = [ang1, vang1, ang2, vang2, ang3, vang3];
    this.x = x;
    this.y = y;
    this.L1 = L1;
    this.L2 = L2;
  ...
  TriplePendulum.prototype.diff = function(Z)
  {
    ...
    ang1_accel = -b1*b23*(g*(M2*b7+M1*(b25+M3)+b21)*Math.sin(b8)+g*M2*b7*Math.sin(b8-2*b9)+2*M2*b35*(b7*(L1*b36*b38+b43)+b49));
    ang2_accel = b54*b23*(g*b55*b65+L1*b65*b38+L2*(M2*b7*Math.sin(b12)-M1*M3*Math.sin(b19))*b42+2*L3*M3*(M2*Math.cos(b8+b16)*b35-M1*b77)*b48);
    ang3_accel = 2*b83*M1*b23*b77*(b7*(b36*(g*b55+L1*b38)+b43)+b49);

    return [vang1, ang1_accel, vang2, ang2_accel, vang3, ang3_accel];
  };
  ...
  TriplePendulum.prototype.draw = function()
  {
    ...
    if (cherrytracer)
    {
      ...
      ctx_tracer.arc(joint3x_draw, joint3y_draw, 4, 0, Math.PI*2, true);
      ctx_tracer.fill();
    }
