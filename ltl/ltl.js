...
document.addEventListener('DOMContentLoaded', function()
{
	...
	document.getElementById("ltl_expression").value = "113 355 / 3533 .0003 / 1 - *";
}, false);
...
function main()
{
	...
	{
		let current_ltl_expression = document.getElementById("ltl_expression").value; // let = temp variable
	...
	if (ltl_expression_changed)
	{
		...
		if (!isNaN(result))
		{
			...
			mermaid.render(syntax_tree.id, formatted_node_list_string, function(svgCode)
			{ 
			    syntax_tree.innerHTML = svgCode;
			    syntax_tree.firstChild.style.height = (Number(syntax_tree.getAttribute('viewbox').split(' ')[3]) + 20) + 'px';
			    syntax_tree.firstChild.style.width = (Number(syntax_tree.getAttribute('viewbox').split(' ')[2]) + 20) + 'px';
			})
		}
		else
		{
