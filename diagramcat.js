...
document.addEventListener("mousemove", evt => {
  ...
  if (mouseDown != null && mouseDown.active && mouseDown.blockId != -1) {
    ...
    fromPos = clientCoordsToSVG(mouseDown.x, mouseDown.y, svg);
    toPos = clientCoordsToSVG(evt.clientX, evt.clientY, svg);
...
sanitizeMermaidSyntax = mermaid_syntax => {
  ...
  mermaid_syntax = mermaid_syntax.replace(regex.allArrows, arrows.join("\n"));
...
updateBlockSelection = () => {
  // Get selection range from textarea to determine which block or arrow is selected
  var textarea = document.getElementById("mermaid_syntax_div");
